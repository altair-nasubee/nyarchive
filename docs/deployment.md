...
- [x] 未ログインで `/` を開くと `/login` にリダイレクトされる
- [x] 「Google でログイン」→ 同意画面 → アプリに戻れる（`ADMIN_EMAIL` のアカウントで）
- [x] `/cats/new` で猫を登録 → 底部バーにアイコンが出る／IDが `No.0001` 形式
- [x] 猫詳細で画像をアップロード → サムネ表示。ブラウザの DevTools で送信ファイルが **WebP・圧縮済み**であること
- [x] アップロード画像が既定で**公開**になり、トップのギャラリーに出る
- [x] 画像を**非公開**に切替 → 別アカウント（別ブラウザ/シークレット）から見えなくなる
- [x] 画像削除・猫削除で、ギャラリーから消える（Blob 実体も削除される）
- [x] **別の Google アカウント**でログイン → 他人の公開画像は見えるが、他人の猫の編集・削除はできない
- [x] `ADMIN_EMAIL` のアカウントは、他人の猫も含め全データを編集・削除でき、非公開画像も見える
- [x] 年齢が誕生日から算出表示され、誕生日未設定でも崩れない
